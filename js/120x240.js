...
app.dpi.module('IntroHorse', ['feed', 'terms', 'documentLoad', 'fonts',
    function (feed, terms) {
        ...
        function IntroHorse(data, onComplete, isFinalScreen, checkLoop) {
            ...
            img.src = boomUrl;
            //if (navigator.userAgent.indexOf('Mac OS X') != -1) {
            //    this.botText.style.paddingTop = '10px';
            //}
        ...
        IntroHorse.prototype.init = function (data, checkLoop) {
            ...
            this.tween
                ...
                // .to(self.longContainer, 0.4, {y: '-=45px', ease: Sine.easeOut}, '-=0')

                .from('#footer', 0.4, {bottom: -45, ease: Sine.easeOut}, '-=.4')
                // .from('#logo', 0.4, {top: -25}, '-=0.4')

                .to(self.topLine, 0.4, {clip: "rect(0px, 200px, 1px, 0px)", ease: Sine.easeOut}, '-=0')
                .to(self.botLine, 0.4, {clip: "rect(0px, 200px, 1px, 0px)", ease: Sine.easeOut}, '-=.5')

                .to(self.topLine, 0.25, {clip: "rect(0px, 200px, 1px, 200px)", ease: Sine.easeIn}, '+=1')
            ...
            .to(this.header, 0.2, {autoAlpha: 1, ease: Sine.easeOut})
            if (this.hasText) {
                this.tween
                    .to(this.topText, 0.35, {clip: " rect(0px, 200px, 50px, 0px)", ease: Sine.easeOut}, '-=0')
                    .to(this.botText, 0.35, {clip: " rect(0px, 200px, 50px, 0px)", ease: Sine.easeOut}, '-=0.35')
            }

            this.tween
                .to(self.assetContainer, 3.5, {
                    x: '+=620px',
                    ease: SlowMo.ease.config(0.031, 0.99, false),
                ...
                //     ease: Sine.easeIn
                // }, '-=0.85')
                .to(self.rail, 0.45, {bottom: 46, ease: Sine.easeOut}, '-=3.2')
                .to(self.rail, 3.3, {
                    x: '-=210px', scale: 1,
                    z: 0.1, /* add this for the jitter bug */
                    rotationZ: "0.01deg", /* add this for the jitter bug */
                ...
                .to(self.leftDirt, 3, {y: '-=50px', x: '+=40', force3D: false, ease: SteppedEase.config(40)}, '-=3')
                .to(self.rightDirt, 3, {
                    y: '-=20px',
                    x: '+=60',
                    force3D: false,
            ...
            if (this.hasText) {
                this.tween
                    .to(this.topText, 0.3, {clip: " rect(0px, 0px, 50px, 0px)", ease: Sine.easeOut}, '-=0.2')
                    .to(this.botText, 0.3, {clip: " rect(0px, 200px, 50px, 200px)", ease: Sine.easeOut}, '-=0.3')
            }

            this.tween
                .to(self.rail, 0.3, {autoAlpha: 0, ease: Sine.easeOut}, '-=1.2')
                .to([self.leftDirt, self.asset], 0.3, {autoAlpha: 0, ease: Sine.easeOut}, '-=0.5')
            ...
            if (this.isFinalScreen) {
                this.tween
                    .to('#header', 0.5, {autoAlpha: 0, ease: Sine.easeOut}, '-=0')
                    .to('#introLogo', 0.5, {autoAlpha: 0, ease: Sine.easeOut}, '-=0.5')
                    .to('#footer', 0.5, {
    ...
    function (feed, terms) {
        ...
        FinalScrOffer2.prototype.init = function (checkLoop) {
            ...
            this.tween
                ...
                .from(this.textBot, 0.5, {top: 210, autoAlpha: 0, ease: Elastic.easeOut.config(0.3, 0.2)}, '-=0.1')
...
app.dpi.module('FinalScr', ['feed', 'terms', 'documentLoad', 'fonts',
    function (feed, terms) {
        ...
        function FinalScr(data, onComplete, isFinalScreen, checkLoop) {
            ...
            if (navigator.userAgent.indexOf('Mac OS X') != -1) {
                if (this.hasDay1) {
                    this.textBot.style.paddingTop = '16px';
                }
            }
...
app.dpi.module('animations', ['feed', 'terms', 'IntroHorse', 'FinalScr', 'FinalScrOffer2', 'FinalScrOffer3', 'documentLoad', 'fonts', 'cta',
    function (feed, terms, IntroHorse, FinalScr, FinalScrOffer2, FinalScrOffer3) {
        ...
        function playFirstScreen() {
            screenIndex = 0;
            TweenLite.delayedCall(0, showTc);

            if (loops > feed.looping) {
                screens[screenIndex].tween.restart();
                screens[screenIndex].tween.play(2.5);
                TweenLite.delayedCall(4, showCta);

                TweenLite.delayedCall(2.3, function () {
                    screens[screenIndex].tween.restart();
                    screens[screenIndex].tween.play(screens[screenIndex].tween.totalDuration() );
                    //console.log(screens[screenIndex].tween.totalDuration() - 0.25);
        ...
        function checkLoop() {
            if (feed.looping === 1) {
                if (feed.screens[screensLength - 1].scrName === 'IntroHorse') {
                    screens[screens.length - 1].tween.pause(-3.3);
